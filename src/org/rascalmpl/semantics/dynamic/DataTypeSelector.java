...
public abstract class DataTypeSelector extends
	...
	static public class Ambiguity extends
		...
		public Ambiguity(org.eclipse.imp.pdb.facts.INode __param1,
				java.util.List<org.rascalmpl.ast.DataTypeSelector> __param2) {
	...
	static public class Selector extends
		...
		public org.eclipse.imp.pdb.facts.type.Type __evaluate(
			...
			if (org.rascalmpl.interpreter.utils.Names.isQualified(sort)) {
				org.rascalmpl.interpreter.env.ModuleEnvironment mod = __eval
						.__getHeap().getModule(
								org.rascalmpl.interpreter.utils.Names
										.moduleName(sort));

				if (mod == null) {
					throw new org.rascalmpl.interpreter.staticErrors.UndeclaredModuleError(
							org.rascalmpl.interpreter.utils.Names
			...
			java.lang.String constructor = org.rascalmpl.interpreter.utils.Names
					.name(this.getProduction());
			java.util.Set<org.eclipse.imp.pdb.facts.type.Type> constructors = __eval
					.__getEnv().lookupConstructor(adt, constructor);

			if (constructors.size() == 0) {
				throw new org.rascalmpl.interpreter.staticErrors.UndeclaredTypeError(
						name + "." + constructor, this);
			} else if (constructors.size() > 1) {
				throw new org.rascalmpl.interpreter.staticErrors.AmbiguousFunctionReferenceError(
						name + "." + constructor, this);
			} else {
